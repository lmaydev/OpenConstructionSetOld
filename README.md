...
This method takes all the required information and uses it to build a data context.
 ...
 - loadGameTypes - if set to `ModLoadType.Active` or `ModLoadType.Base` the game's data files will be loaded into the context as specified. Passing `ModLoadType.None` will cause them not to be loaded.
