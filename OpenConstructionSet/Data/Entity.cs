    ...
    public class Entity : IRevertibleChangeTracking, IClearable
    {
        ...
        public void Update(Item item)
        {
            ...
            foreach (var pair in item.References)
            {
                ...
                if (!ReferenceCategories.TryGetValue(category, out OcsDictionary<ReferenceValues> references))
                {
        ...
        public Item GetChanges(bool newItem)
        {
            ...
            if (newItem)
            {
                changes = ItemChanges.New;
            }
            else if (properties.Modified(nameof(Name)))
            {
                changes = ItemChanges.Renamed;
            }
            else
            {
                changes = ItemChanges.Changed;
            }
