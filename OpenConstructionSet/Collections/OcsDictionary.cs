...
public class OcsDictionary<T> : IDictionary<string, T>, IReadOnlyDictionary<string, T>, IRevertibleChangeTracking, IClearable, IOcsDictionary<T>
{
    ...
    public OcsDictionary()
    {
        ...
        (var tracking, var revert) = CheckChildType();
    ...
    {
        var sucess = currentValues.Remove(key);
    ...
    public void CopyTo(KeyValuePair<string, T>[] array, int arrayIndex) => ((IDictionary<string, T>)currentValues).CopyTo(array, arrayIndex);

    public IEnumerator<KeyValuePair<string, T>> GetEnumerator() => currentValues.GetEnumerator();

    public bool TryGetValue(string key, [MaybeNullWhen(false)] out T value) => currentValues.TryGetValue(key, out value);
